...
int get_student(int fd, int id, student_t *s)
{
    ...
    while ((bytes_read = read(fd, &tmp, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE)) {
        if (tmp.id == id) {
            *s = tmp;
            return NO_ERROR;
        }
...
int del_student(int fd, int id)
{
    ...
    while ((position = lseek(fd, 0, SEEK_CUR)), read(fd, &tmp, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (tmp.id == id) {
...
int count_db_records(int fd)
{
    ...
    if (fprintf(stdout, count > 0 ? M_DB_RECORD_CNT : M_DB_EMPTY, count) < 0) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    } 
...
int print_db(int fd)
{
    ...
    while ((bytes_read = read(fd, &tmp, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
        if (tmp.id != DELETED_STUDENT_ID) {
            ...
            printf(STUDENT_PRINT_FMT_STRING, tmp.id, tmp.fname, tmp.lname, tmp.gpa / 100.0);
            found = 1;
        }
    }
